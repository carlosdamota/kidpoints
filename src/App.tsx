...
function App() {
  const { authReady, user, needsOnboarding, state, activeChild, handleOnboardingComplete } = useFamily();
  const [activeTab, setActiveTab] = useState<Tab>('misiones');

  if (!authReady) {
    return <div className="min-h-screen flex items-center justify-center text-cyan-neon">Cargando sistemas...</div>;
  }
